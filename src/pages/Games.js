...
function Games() {
  ...
  useEffect(() => {
    const fetchGame = async () => {
      ...
      try {
        ...
        const gameData = await wordpressApi.posts.getByPostTypeAndSlug('games', decodedSlug, true);
        setGame(gameData);
      } catch (error) {
        console.error('Error fetching game:', error);
        // Check if it's a 404 error (game not found)
        if (error.message && error.message.includes('not found')) {
          console.warn(`Game with slug "${slug}" not found in WordPress`);
        }
        setNotFound(true);
      } finally {
        setLoading(false);
  ...
  return (
    <div className="pt-[200px] p-4 container mx-auto">
        <h1 className="text-4xl font-bold mb-4 text-white" dangerouslySetInnerHTML={{ __html: game.title }}></h1>

        <hr className="border-t border-t-gray-60 mb-4"/>

        <div className="text-gray-400 flex-wrap mb-5 lg:mb-0 flex justify-between">
            <div>
                Posted by <Link to="/profile" className="w-full ">{game.authorName || 'Author'}</Link> on {formatDate(game.date)}
            </div>
        ...
        <div>
            <div className="grid gap-5 lg:grid-cols-5">
                <div className="lg:col-span-3">
                    {game.image && (
                        <Image url={game.image} alt={game.title} />
                    )}
                    <div 
